    ...
    def set_data(self):
        ...
        if self.dataset == 'DIGIT':
            ...
            for _ in range(self.n_client):
                clnt_x.append(list())
                clnt_y.append(list())
                clnt_val_x.append(list())
                clnt_val_y.append(list())
            ...
            for client_idx, curruption_name in enumerate(curruption_name_list):
                ...
                np.random.seed(self.seed + client_idx)
                ...
                train_label = [label[i] for i in train_indices]
                val_data = [data[i] for i in val_indices]
                val_label = [label[i] for i in val_indices]
                ...
                for img in train_data:
                    ...
                    img = np.array(img)
                    _data.append(img)
                train_data = np.asarray(_data)
                train_data = train_data.astype(np.float32) / 255
                train_data = (train_data - mean) / std
                train_data = train_data.transpose((0, 3, 1, 2))
                clnt_x[client_idx].extend(train_data)
                clnt_y[client_idx].extend(train_label)
                ...
                for img in val_data:
                    ...
                    img = np.array(img)
                    _data.append(img)
                val_data = np.asarray(_data)
                val_data = val_data.astype(np.float32) / 255
                val_data = (val_data - mean) / std
                val_data = val_data.transpose((0, 3, 1, 2))
                clnt_val_x[client_idx].extend(val_data)
                clnt_val_y[client_idx].extend(val_label)
            ...
            for client_idx in range(self.n_client):
                ...
                clnt_val_x[client_idx] = np.asarray(clnt_val_x[client_idx])
                clnt_val_y[client_idx] = np.asarray(clnt_val_y[client_idx])
        ...
        self.clnt_x = clnt_x
        self.clnt_y = clnt_y
        self.clnt_val_x = clnt_val_x
        self.clnt_val_y = clnt_val_y
        self.tst_x = tst_x
        self.tst_y = tst_y
        ...
        for clnt in range(self.n_client):
            print("Client %3d Train: " % clnt +
                  ', '.join(["%.3f" % np.mean(self.clnt_y[clnt] == cls) for cls in range(self.n_cls)]) +
                  ', Amount:%d' % len(self.clnt_y[clnt]))
            print("Client %3d Val: " % clnt +
                  ', '.join(["%.3f" % np.mean(self.clnt_val_y[clnt] == cls) for cls in range(self.n_cls)]) +
                  ', Amount:%d' % len(self.clnt_val_y[clnt]))
            count += len(self.clnt_y[clnt])
        print('Total Train Amount:%d' % count)
        print('--------')
        print("Test: " +
