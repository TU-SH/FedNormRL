...
def train_model_fedavg(model, trn_x, trn_y, val_x, val_y, learning_rate, batch_size, epoch, weight_decay, dataset_name, sch_step=1, sch_gamma=1):
    n_trn = len(trn_x)
    ...
    val_gen = data.DataLoader(Dataset(val_x, val_y, dataset_name=dataset_name), batch_size=batch_size, shuffle=False, num_workers=2)
    ...
    for e in range(epoch):
        ...
        for i in range(int(np.ceil(n_trn / batch_size))):
            ...
            # Compute validation loss before update
            val_loss_before, _ = get_acc_loss(val_x, val_y, model, dataset_name, weight_decay)

            y_pred, rl_data = model(batch_x, loss=val_loss_before, comm_round=None)
            ...
            # Compute validation loss after update
            val_loss_after, _ = get_acc_loss(val_x, val_y, model, dataset_name, weight_decay)
            reward = -(val_loss_after - val_loss_before)  # Negative loss difference as reward

            # Update RL agents in AGN modules
            if rl_data is not None:
                for state, action in rl_data:
                    next_state = model.bn1.compute_state(batch_x, loss=val_loss_after, comm_round=None)
                    model.bn1.update_rl(state, action, reward, next_state)
                    model.bn2.update_rl(state, action, reward, next_state)
...
def train_FedAvg(args, data_obj, model_func, init_model):
    ...
    clnt_val_x = data_obj.clnt_val_x
    clnt_val_y = data_obj.clnt_val_y
    ...
    for i in range(com_amount):
        ...
        for clnt in selected_clnts:
            ...
            trn_x = clnt_x[clnt]
            trn_y = clnt_y[clnt]
            val_x = clnt_val_x[clnt]
            val_y = clnt_val_y[clnt]
            ...
            clnt_models[clnt] = train_model_fedavg(clnt_models[clnt], trn_x, trn_y, val_x, val_y,
